            ...
            <div class="container-fluid mx-5"> <!-- Menambahkan mx-3 di sini -->
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav ms-auto navbar-dark">
                     ...
                     <!-- Search form -->
                     <form class="d-flex" action="{{ route('cariofficial') }}" method="GET" class="mb-4">
                    ...
                    <ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start"
                        ...
                        <li>
                            ...
                            <ul class="collapse nav flex-column ms-1" id="submenu3" data-bs-parent="#menu">
                                ...
                                <li>
                                    <a href="#" class="nav-link px-0"> <span class="d-none d-sm-inline">Wasit</span>
                                    </a>
                                </li>
                            </ul>
                        </li>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            @foreach ($data_official as $item)
                                                <tr>
                                                    <td>{{ $item->tanggal_official }}</td>
                                                    <td>{{ $item->nama_official }}</td>
                                                    <td>{{ $item->klub_official }}</td>
                                                    <td>{{ $item->status_official }}</td>
                                                    <td>
                                                        <a class="btn btn-secondary btn-sm me-1"
                                                            href='{{ url('/adminofficial/' . $item->id) }}'>Detail</a>
                                                        <a class="btn btn-warning btn-sm"
                                                            href='{{ route('editofficial', ['id' => $item->id]) }}'>Edit</a>


                                                        <form onsubmit="return confirm('Apakah ingin Hapus')"
                                                            class="d-inline" action="{{ '/adminofficial/' . $item->id }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
